...
const hypercare = computed(() => store.current?.hypercare || null)
const status = computed(() => store.current?.hypercare_status || null)
const incidents = computed(() => store.current?.incidents.filter((i) => i.stage_number === STAGE_HYPERCARE_NUMBER) || [])
...
async function saveNotes() {
  ...
  await store.updateHypercare(props.deploymentId, { notes: notes.value })
...
async function addIncident() {
  ...
  const res = await store.addIncident(props.deploymentId, {
    stage_number: STAGE_HYPERCARE_NUMBER,
    ...
    <v-card>
      ...
      <v-card-text style="max-height: 70vh">
        ...
        <p v-else class="text-body-2 textSecondary mb-4">
          Hypercare hasn't started yet — it begins automatically once Stage 11 (Go-Live) is complete.
        </p>
        ...
        <v-btn color="primary" variant="tonal" :loading="savingNotes" :disabled="!hypercare" @click="saveNotes">
          Save notes
        </v-btn>
      </v-card-text>
    </v-card>
