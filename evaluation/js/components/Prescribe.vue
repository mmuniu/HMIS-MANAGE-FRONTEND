<template>
          ...
          <v-btn icon dark @click.native="dialogVisible=false" absolute right>
            ...
            <v-card-text>
              ...
              <!-- First section -->
                <v-row>
                  ...
                  <v-col cols="7" class="small-font">
                    ...
                    >
                      ...
                      <!-- No data slot (only when no matching results) -->
                      <template v-slot:no-data v-if="product.search.own_results.length === 0 && (product.search.own_term && product.search.own_term.length > 2)">
                ...
                <v-row>
                  <v-col cols="3">
                    <v-text-field label="Duration" outline v-model="prescription.own_duration"
      ...
      <v-row>
        ...
        <v-col cols="7" class="small-font">
          <v-combobox
            ...
            @change="selectedPrescription(prescription.product)"
            chips
  ...
  ></v-text-field>
      ...
      <v-row>
        ...
        <v-col cols="3">
          <v-text-field label="Quantity" v-model="prescription.quantity" @change="calculateDispensableUnit()"></v-text-field>
...
import OwnPrescription from '@evaluation/libs/notes/OwnPrescription';
...
export default {
    ...
    data() {
        return {
            ...
            ownprescription: new OwnPrescription(),
    ...
    computed: {
        ...
        contaminated() {
            return this.prescription?.form?.errorDetected || false;
        },
        ...
        selectedOwnProduct() {
            return this.prescription.own_product;
    ...
    own_term(value) {
                if (value) {
                    if (value.length > 2) {
                        this.placeholder = '';
                        this.product.own_index(this.prescription.own_department_id, this.prescription.own_scheme_id, this.outpatient);
                    } else {
                        this.placeholder = '(Enter ' + (3 - value.length) + ' more characters)';
                        this.product.search.own_reset();
                    }
                }
     },
        ...
        visit(value) {
            if (value) {
                ...
                this.prescription.own_visit= value.id;
    ...
    'product.dispensable_amount': function(newVal) {
      this.calculateDispensableUnit();
        ...
        treatmentLocation(value) {
            ...
            this.prescription.own_treatment_location_id =value.id;
        },
  ...
  if (!isNaN(this.prescription.quantity) && !isNaN(data.dispensable_amount)) {
    ...
    selectedOwnDrug(value) {
      // Handle when a drug is selected from the autocomplete
    },
    ...
    try {
        ...
        // Close the dialog if successful
              ...
              this.resetOwnPrescriptionForm();
                    ...
                    // Handle the error (optional logging or displaying the error)
                    console.error(error);
                } finally {
