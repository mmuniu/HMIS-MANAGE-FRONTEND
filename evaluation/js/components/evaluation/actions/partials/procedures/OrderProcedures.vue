...
export default {
  props: [
    'items', 'domain', 'visit_identifier', 'destination', 'admission_id',
  ],
  ...
  methods: {
    ...
    async checkout() {
      if (this.cart.consumable_states ? window.confirm('Some required consumables or drugs are less than required. Are you sure you still want to order the procedure?') : true) {
        ...
        const response = await this.procedure.orderProcedures(this.cart,
  ...
  async mounted()  {
    ...
    // Set categories based on the items passed in props
    if (this.checkCurrentModule() == 'inpatient' &&  this.checkCurrentTab() == 'procedures') {
    ...
    }else if (this.checkCurrentModule() == 'inpatient' &&  this.checkCurrentTab() == 'investigations') {
      console.log('557')
