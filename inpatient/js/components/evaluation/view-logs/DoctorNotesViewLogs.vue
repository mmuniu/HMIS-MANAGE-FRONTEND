            ...
            <v-row>
              <v-col cols="12">
                <v-simple-table>
                  ...
                  <tbody>
                    <tr v-for="note in doctorNotesLogs" :key="note.id">
...
export default {
    ...
    const viewLogs = await this.history.fetchDoctorNotesLogs(visit_id);

const notes = viewLogs?.notes || []; // Access notes data array
  // Map through notes and enrich with logs
  this.doctorNotesLogs = notes.map(note => {
      return {
