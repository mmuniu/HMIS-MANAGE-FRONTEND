    ...
    <div>
        ...
        <v-card class="mt-3">
            <v-card-title class="info">
                ...
                <v-text-field
                        v-if="msetting('inpatient.browser_search_dispatches')"
                        v-model="searchLocal"
                        append-icon="search"
                ...
                <div v-else>
                    ...
                    <div v-if="getAdmissions && getAdmissions.data">
                        ...
                        <v-data-table
                                :headers="headers"
                                :items="dispatches"
                                :hide-default-footer="!msetting('inpatient.browser_search_dispatches')"
                                :search="searchLocal"
                                :loading="saveLoader"
                                class="mb-3"
                                flat
                        >
                            ...
                            <template v-slot:item.actions="{ item }">
                               ...
                               <!--   <v-btn color="warning" @click="editAdmissionDetails(item)" small>
                                    edit Adm. Details
                                </v-btn>

                                <v-btn color="purple" dark @click="changeWards(item)" small :disabled="! wards">
                ...
                <v-card-text>
                    <v-container fluid>
                        ...
                        <v-row>
                            ...
                            <v-col cols="9">
                                <collabmed-date-time-picker dateOnly outline  v-model="obj.editAdmissionDetails.admission_date" label=""></collabmed-date-time-picker>
                            </v-col>
                        </v-row>
                    </v-container>
                </v-card-text>
...
<script>
// import Admission from '@inpatient/libs/dispatches/Admission';
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: 'Condition', value: 'body_condition' },
                { text: 'Brought By', value: 'brought_by' },
                { text: 'Fridge', value: 'fridge' },
                { text: 'Cabinet', value: 'cabinet' },
                { text: 'Admitted on', value: 'created_at' },
    ...
    methods: {
        ...
        printReservationForm(item) {
            return route('inpatient.dispatches.reservation-form.print', item.id).relative();
        },
    ...
    async mounted() {
        this.initialise();
        const response = await fetch('/api/morgue/all/dispatches'); // Adjust API endpoint as needed
