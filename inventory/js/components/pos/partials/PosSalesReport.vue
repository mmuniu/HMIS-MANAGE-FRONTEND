    ...
    <div v-if="initialised">
        ...
        <div v-if="!visit_id">
            <v-card-title class="subheading grey-text text-darken-2">
                <h2 class="text-body-2 ma-0 pa-0">All Pos sales details report </h2>
                <v-spacer></v-spacer>
...
export default {
    ...
    methods: {
      ...
      uniqueSales() {
        ...
        url() {
            const params = 'station_id=' + this.station.id + '&start_date=' + this.filters.start_date + '&end_date=' + this.filters.end_date + '&status=' + this.filters.status;
      ...
      async init() {
        const url = this.url(); // Call the url method to get the complete URL
        try {
