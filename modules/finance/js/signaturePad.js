    ...
    var buttonSign = document.getElementById("SignBtn");
    ...
    function onMessage(evt) {
        ...
        if (obj.TOKEN_TYPE == "TOKEN_TYPE_SEND") {
            ...
            if ((obj.TOKEN_CMD_ORIGIN == "TOKEN_CMD_SIGNATURE_CONFIRM") || (obj.TOKEN_CMD_ORIGIN == "TOKEN_CMD_API_SIGNATURE_CONFIRM")) {
                ...
                document.getElementById("Signature_0").src = SIGNED_SIGNATURE = "data:image/png;base64," + obj.TOKEN_PARAM_FILE;
    ...
    */
    ...
    function getSignature() {
        ...
        document.getElementById("Signature_0").src = "/modules/finance/img/signaturePad/White.png";
    ...
    function open_pad() {
        ...
        buttonSign.value = "[AWAITING PATIENT CONFIRMATION!]";
    ...
    function selection_confirm() {
        if (websocketMode == websocketModes.Default) {
            ...
            for (i = 1; i <= document.getElementById("chek_boxes_selectedElements").value; i++) {
                ...
                status += 'Patient Accepted Invoice Amount (Kshs. ' + INVOICE_AMOUNT + ') \n Awaiting signature entry \n';
            }
    ...
    function selection_cancel() {
        if (websocketMode == websocketModes.Default) {
            ...
            alert("The Patient Rejected the Invoice Amount!!!");
    ...
    {
        toastr.success('Signature Acquired. Now Saving the Signature');

        let data = {
            patient_signature: SIGNED_SIGNATURE
        };

        let theText = buttonSign.value;
        buttonSign.value = "[SAVING SIGNATURE NOW]";

        let url = route('api.finance.invoices.update-signature', INVOICE_ID).relative();

        axios["post"](url, data, visa())
        .then(({data}) => {

            buttonSign.value = "[SIGNATURE SAVED!]";
            toastr.success('Signature has been saved successfully. Refresh the page now');
        ...
        .catch(({response}) => {
            ...
            alert("Signature could not be saved!! \nPlease try again!")
        });
