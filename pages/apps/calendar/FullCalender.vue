...
export default defineComponent({
  ...
  if (event) {
    // Convert start and end to Date objects if they are not null or empty
    const updatedStart = this.selectedEvent.start
      ? new Date(this.selectedEvent.start)
      : null;
    const updatedEnd = this.selectedEvent.end
      ? new Date(this.selectedEvent.end)
      : null;

    if (isNaN(updatedStart.getTime()) || (updatedEnd && isNaN(updatedEnd.getTime()))) {
      console.error("Invalid start or end date.");
      return; // Prevent updating if the dates are invalid
    }
    ...
    // Close the update modal and reset values
