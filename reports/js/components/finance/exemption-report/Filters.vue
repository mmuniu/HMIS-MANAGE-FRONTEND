...
export default {
  ...
  computed: {
    ...
    url() {
      return '/api/reports/module/finance/exemption-report';
    },
  ...
  data: () => ({
    loading: false,
    filename: 'Exemptions Report for ' + moment(new Date()).format('YYYY-MM-DD'),
    filters: {
      facility_id: null,
      ward_id: null,
      start_date: null,
  ...
  created() {
    // Set default values to today's date and time only once during component creation
    this.filters.start_date = moment().format('YYYY-MM-DD HH:mm');
    this.filters.end_date = moment().format('YYYY-MM-DD HH:mm');
  },
